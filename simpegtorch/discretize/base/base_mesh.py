...
class BaseMesh:
    ...
    def equals(self, other_mesh):
        ...
        for item in self._items:
            ...
            if isinstance(my_attr, torch.Tensor):
                is_equal = torch.allclose(my_attr, other_mesh_attr, rtol=0, atol=0)
            elif isinstance(my_attr, tuple):
                is_equal = len(my_attr) == len(other_mesh_attr)
                if is_equal:
                    for thing1, thing2 in zip(my_attr, other_mesh_attr):
                        if isinstance(thing1, torch.Tensor):
                            is_equal = torch.allclose(thing1, thing2, rtol=0, atol=0)
                        else:
                            try:
    ...
    def cell_centers(self):
        ...
        >>> hx, hy, hz = torch.ones(2), 2*torch.ones(2), 3*torch.ones(2)
    ...
    def faces(self):
        ...
        the z-faces; i.e. *mesh.faces* is equivalent to *torch.tensor([mesh.faces_x, mesh.faces_y, mesh.face_z])* .
    ...
    def edges(self):
        ...
        the z-edges; i.e. *mesh.edges* is equivalent to *torch.tensor([mesh.edges_x, mesh.edges_y, mesh.edges_z])* .
    ...
    def project_face_vector(self, face_vectors):
        ...
        *project_face_vector* computes the dot product between
        ...
        """
        if not isinstance(face_vectors, torch.Tensor):
            raise Exception("face_vectors must be an Tensor")
        if not (
            len(face_vectors.shape) == 2
            and face_vectors.shape[0] == self.n_faces
            and face_vectors.shape[1] == self.dim
        ):
            raise Exception("face_vectors must be an Tensor of shape (n_faces, dim)")
        return torch.sum(face_vectors * self.face_normals, 1)
    ...
    def project_edge_vector(self, edge_vectors):
        ...
        *project_edge_vector* computes the dot product between
        ...
        return torch.sum(edge_vectors * self.edge_tangents, 1)
    ...
    def face_divergence(self):
        ...
        *See notes for additional details.*
        ...
        >>> ux = (faces_x[:, 0] / torch.sqrt(torch.sum(faces_x ** 2, axis=1))) * torch.exp(
        ...     -(faces_x[:, 0] ** 2 + faces_x[:, 1] ** 2) / 6 ** 2
        ... )
        >>> uy = (faces_y[:, 1] / torch.sqrt(torch.sum(faces_y ** 2, axis=1))) * torch.exp(
        ...     -(faces_y[:, 0] ** 2 + faces_y[:, 1] ** 2) / 6 ** 2
        ... )
        >>> u = torch.tensor([ux, uy])
    ...
    def nodal_gradient(self):
        ...
        >>> nodes = mesh.nodes
        >>> phi = torch.exp(-(nodes[:, 0] ** 2 + nodes[:, 1] ** 2) / 4 ** 2)
    ...
    def edge_curl(self):
        ...
        >>> edge_ind = np.where(
        ...     torch.sum((mesh.edges-mesh.faces[face_ind, :])**2, axis=1) <= 0.5 + 1e-6
        ... )[0]
        ...
        ...     poly = node + torch.cat([torch.tensor([0, 0, 0, 0]), torch.tensor([0, 1, 1, 0]), torch.tensor([0, 0, 1, 1])])
        ...     ds = [0.07, -0.07, -0.07]
        ... elif face_norm[1] == 1:
        ...     k = (edges[:, 0] == max_edges[0]) | (edges[:, 2] == min_edges[2])
        ...     poly = node + torch.cat([torch.tensor([0, 1, 1, 0]), torch.tensor([0, 0, 0, 0]), torch.tensor([0, 0, 1, 1])])
        ...     ds = [0.07, -0.09, -0.07]
        ... elif face_norm[2] == 1:
        ...     k = (edges[:, 0] == min_edges[0]) | (edges[:, 1] == max_edges[1])
        ...     poly = node + torch.cat([torch.tensor([0, 1, 1, 0]), torch.tensor([0, 0, 1, 1]), torch.tensor([0, 0, 0, 0])])
        ...     ds = [0.07, -0.09, -0.07]
        >>> edge_tan[k, :] *= -1
    ...
    def stencil_cell_gradient(self):
        ...
        >>> phi = torch.zeros(mesh.nC)
        >>> k = (torch.abs(mesh.cell_centers[:, 0]) < 10.) & (torch.abs(mesh.cell_centers[:, 1]) < 10.)
        >>> phi[k] = 1.
    ...
    def get_face_inner_product(
        ...
        **kwargs,
        ...
        model : None or numpy.ndarray, optional
            ...
            - *(n_cells, dim)* :class:`numpy.ndarray` : returns inner product matrix for
              diagonal anisotropic case. Columns are ordered ``torch.cat([σ_xx, σ_yy, σ_zz])``.
            ...
            - *(n_cells, 3)* :class:`numpy.ndarray` (``dim`` is 2) or
              *(n_cells, 6)* :class:`numpy.ndarray` (``dim`` is 3) :
              ...
              ordered ``torch.cat([σ_xx, σ_yy, σ_zz, σ_xy, σ_xz, σ_yz])`` This can also be a
        ...
        **Tensor Case:** the inner product between :math:`\vec{u}` and :math:`\Sigma \vec{w}`
        ...
        >>> sig_iso_tensor = sig1 * torch.eye(3)
        >>> sig_diag_tensor = sdiag(torch.tensor([sig1, sig2, sig3]))
        >>> sig_full_tensor = torch.tensor([
        ...     [sig1, sig4, sig5],
        ...     [sig4, sig2, sig6],
        ...     [sig5, sig6, sig3]
        ... ])
        ...
        >>> v = torch.ones(mesh.nC)
        >>> sig = sig1 * v
        >>> M1 = mesh.get_face_inner_product(sig)

        Diagonal anisotropic case:

        >>> sig = torch.cat([sig1*v, sig2*v, sig3*v])
        >>> M2 = mesh.get_face_inner_product(sig)

        Full anisotropic case:

        >>> sig = torch.tile(torch.cat([sig1, sig2, sig3, sig4, sig5, sig6]), (mesh.nC, 1))
    ...
    def get_edge_inner_product_surface(
        ...
        **get_edge_inner_product_suface** constructs the inner product matrix :math:`\mathbf{M_\tau}`
        ...
        >>> tau = torch.tensor([)
        >>>     tau_x * torch.ones(mesh.n_faces_x),
        >>>     tau_y * torch.ones(mesh.n_faces_y),
        >>>     tau_z * torch.ones(mesh.n_faces_z)
        >>> ]
    ...
    def get_face_inner_product_surface(
        ...
        **get_face_inner_product_suface** constructs the inner product matrix :math:`\mathbf{M_\tau}`
        ...
        if model is None:
            model = torch.ones(self.nF)

        if invert_model:
            model = 1.0 / model

        if is_scalar(model):
            model = model * torch.ones(self.nF)
    ...
    def get_edge_inner_product_line(
        ...
        **get_edge_inner_product_line** constructs the inner product matrix :math:`\mathbf{M_\lambda}`
        ...
        >>> tau = torch.tensor([)
        >>>     tau_x * torch.ones(mesh.n_edges_x),
        >>>     tau_y * torch.ones(mesh.n_edges_y),
        >>>     tau_z * torch.ones(mesh.n_edges_z)
        >>> ]
        ...
        if model is None:
            model = torch.ones(self.nE)

        if invert_model:
            model = 1.0 / model

        if is_scalar(model):
            model = model * torch.ones(self.nE)
        ...
        **get_face_inner_product_deriv** constructs a function handle for computing
        ...
        model : numpy.ndarray
            ...
            - *(n_cells, dim)* :class:`numpy.ndarray` : Diagonal anisotropic case.
              Columns are ordered ``torch.cat([σ_xx, σ_yy, σ_zz])``. This can also a be a 1D
            ...
            - *(n_cells, 3)* :class:`numpy.ndarray` (``dim`` is 2) or
              *(n_cells, 6)* :class:`numpy.ndarray` (``dim`` is 3) : Full tensor properties case. Columns
              are ordered ``torch.tensor([σ_xx, σ_yy, σ_zz, σ_xy, σ_xz, σ_yz])`` This can also be
        ...
        **get_edge_inner_product_deriv** constructs a function handle for computing
        ...
        model : numpy.ndarray
            ...
            - *(n_cells)* :class:`numpy.ndarray` : Isotropic case. *model* contains a
              scalar physical property value for each cell.
            - *(n_cells, dim)* :class:`numpy.ndarray` : Diagonal anisotropic case.
              Columns are ordered ``torch.tensor([σ_xx, σ_yy, σ_zz])``. This can also a be a 1D
              array with the same number of total elements in column major order.
            - *(n_cells, 3)* :class:`numpy.ndarray` (``dim`` is 2) or
              *(n_cells, 6)* :class:`numpy.ndarray` (``dim`` is 3) : Full tensor properties case. Columns
              are ordered ``torch.tensor([σ_xx, σ_yy, σ_zz, σ_xy, σ_xz, σ_yz])`` This can also be
    ...
    def get_face_inner_product_surface_deriv(
        ...
        **get_face_inner_product_surface_deriv** constructs a function handle for computing
        ...
        if tensorType == 0:  # isotropic, constant
            indices = torch.stack(
                [
                    torch.arange(self.nF, dtype=torch.long),
                    torch.zeros(self.nF, dtype=torch.long),
                ],
                dim=0,
            )

            values = torch.ones(self.nF)

            ones = torch.sparse_coo_tensor(indices, values, size=(self.nF, 1))
    ...
    def get_edge_inner_product_line_deriv(
        ...
        **get_edge_inner_product_line_deriv** constructs a function handle for computing
        ...
        if tensorType == 0:  # isotropic, constant
            row_indices = torch.arange(self.nE, dtype=torch.long)
            col_indices = torch.zeros(self.nE, dtype=torch.long)
            indices = torch.stack([row_indices, col_indices], dim=0)

            values = torch.ones(self.nE)
            ones = torch.sparse_coo_tensor(indices, values, size=(self.nE, 1))
            if not invert_matrix and not invert_model:
                dMdprop = L * ones
            elif invert_matrix and invert_model:
    ...
    def average_face_to_cell_vector(self):
        ...
        **average_face_to_cell_vector** constructs a discrete linear operator
        ...
        the form torch.tensor([ux, uy, uz]. For each cell, and for each Cartesian component,)
        ...
        >>> faces_x = mesh.faces_x
        >>> faces_y = mesh.faces_y
        >>> u_fx = -(faces_x[:, 1] / torch.sqrt(torch.sum(faces_x ** 2, axis=1))) * torch.exp(
        >>>     -(faces_x[:, 0] ** 2 + faces_x[:, 1] ** 2) / 6 ** 2
        >>> )
        >>> u_fy = (faces_y[:, 0] / torch.sqrt(torch.sum(faces_y ** 2, axis=1))) * torch.exp(
        >>>     -(faces_y[:, 0] ** 2 + faces_y[:, 1] ** 2) / 6 ** 2
        >>> )
        >>> u_f = torch.tensor([u_fx, u_fy])
    ...
    def average_cell_vector_to_face(self):
        ...
        **average_cell_vector_to_face** constructs a discrete linear operator
        ...
        >>> centers = mesh.cell_centers
        >>> u_x = -(centers[:, 1] / torch.sqrt(torch.sum(centers ** 2, axis=1))) * torch.exp(
        ...     -(centers[:, 0] ** 2 + centers[:, 1] ** 2) / 6 ** 2
        ... )
        >>> u_y = (centers[:, 0] / torch.sqrt(torch.sum(centers ** 2, axis=1))) * torch.exp(
        ...     -(centers[:, 0] ** 2 + centers[:, 1] ** 2) / 6 ** 2
        ... )
        >>> u_c = torch.tensor([u_x, u_y])
    ...
    def average_edge_to_cell_vector(self):
        ...
        **average_edge_to_cell_vector** constructs a discrete linear operator
        ...
        >>> u_ex = -(edges_x[:, 1] / torch.sqrt(torch.sum(edges_x ** 2, axis=1))) * torch.exp(
        ...     -(edges_x[:, 0] ** 2 + edges_x[:, 1] ** 2) / 6 ** 2
        ... )
        >>> u_ey = (edges_y[:, 0] / torch.sqrt(torch.sum(edges_y ** 2, axis=1))) * torch.exp(
        ...     -(edges_y[:, 0] ** 2 + edges_y[:, 1] ** 2) / 6 ** 2
        ... )
        >>> u_e = torch.tensor([u_ex, u_ey])
    ...
    def average_edge_to_face(self):
        ...
        >>> edges = mesh.edges
        >>> u_ex = -(edges[:, 1] / torch.sqrt(torch.sum(edges ** 2, axis=1))) * torch.exp(
        ...     -(edges[:, 0] ** 2 + edges[:, 1] ** 2) / 6 ** 2
        ... )
        >>> u_ey = (edges[:, 0] / torch.sqrt(torch.sum(edges ** 2, axis=1))) * torch.exp(
        ...     -(edges[:, 0] ** 2 + edges[:, 1] ** 2) / 6 ** 2
        ... )
        >>> u_e = np.c_[u_ex, u_ey]
    ...
    def closest_points_index(self, locations, grid_loc="CC", discard=False):
        ...
        """
        if KDTree is None:
            raise ModuleNotFoundError("scipy must be installed to use this function")
